...
class ClassSessionController extends Controller
{
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Crear slots de disponibilidad para un profesor
    |--------------------------------------------------------------------------
    */
    public function createAvailabilitySlots(Request $request)
    {
        $request->validate([
            'teacher_id' => 'required|integer',
            'town_id'    => 'required|integer',
        ...
        while ($current->lte($dateTo)) {
            ...
            try {
                // Validar que el profesor esté asignado al pueblo
                $teacherTown = TeacherTown::where('teacher_profile_id', $teacherId)
                    ->where('town_id', $townId)
                    ->first();
                ...
                if (!$teacherTown) {
                    $errors[] = [
                        'date' => $date,
                        'error' => 'Profesor no asignado a este pueblo'
                    ];
                    $current->addDay();
                    continue;
                }
                ...
                if (!$availability) {
                    $errors[] = [
                        'date' => $date,
                        'error' => 'Sin disponibilidad semanal para este día'
                    ];
                    $current->addDay();
                    continue;
                }

                // Obtener vehículo válido
                $vehicle = TeacherVehicle::getVehicleForDate($teacherId, $date);
                if (!$vehicle) {
                    $errors[] = [
                        'date' => $date,
                ...
                $slots = [];
                $cursor = Carbon::parse($date . ' ' . $availability->starts_time);
                $end = Carbon::parse($date . ' ' . $availability->end_time);
        ...
        return response()->json([
            ...
            'errors' => $errors,
            'summary' => [
                'total_days_processed' => $dateFrom->diffInDays($dateTo) + 1,
                'days_with_slots' => count($createdSlots),
                'total_slots_generated' => collect($createdSlots)->sum('total_slots'),
                'days_with_errors' => count($errors),
            ]
        ]);
    }
